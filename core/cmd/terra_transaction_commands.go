...
// TerraSendCoins transfers coins from the node's account to a specified address.
func (cli *Client) TerraSendCoins(c *cli.Context) (err error) {
	if c.NArg() < 3 {
		return cli.errorOut(errors.New("sendether expects three arguments: amount, fromAddress and toAddress"))
	}

	coins, err := sdk.ParseDecCoins(c.Args().Get(0))
	if err != nil {
